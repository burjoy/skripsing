      ...
      if(write_or_not == 1){
        ...
        if(twai_receive(&message_inverter_speed_rpm_temp, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_inverter_speed_rpm_temp.identifier == 0x0CF00401){
                ...
                speedy = waktu + " " + String(real_speed) + "\t\t" + "engine_speed";
                ...
                rpms = waktu + " " + String(real_rpm) + "\t\t" + "engine_rpm";
                ...
                temps = waktu + " " + String(engine_temp) + "\t\t" + "engine_temp";
                // if(currentMillis - prevMillis >= 7000){
                //   File file = SD.open(inverter_pdu_state_now_cstr, FILE_APPEND);
                //   if (!file) {
                //     Serial.println("File not exist yet");
        ...
        if(twai_receive(&message_inverter_speed_rpm_temp, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_inverter_speed_rpm_temp.identifier == 0x0CF00402){
                printf("\nID Inverter: %x", message_inverter_speed_rpm_temp.identifier);        
                uint8_t voltage_msb = message_inverter_speed_rpm_temp.data[1];
                uint8_t voltage_lsb = message_inverter_speed_rpm_temp.data[0];
                inverter_voltage = (voltage_msb << 8) | voltage_lsb;
                // int real_speed = engine_speed * 0.125;
                tegangan = waktu + " " + inverter_voltage + "\t\t" + "inverter_voltage";

                uint8_t current_msb = message_inverter_speed_rpm_temp.data[3];
                uint8_t current_lsb = message_inverter_speed_rpm_temp.data[2];
                inverter_current = (current_msb << 8) | current_lsb;
                // int real_rpm = engine_rpm * 0.125;
                arus = waktu + " " + inverter_current + "\t\t" + "inverter_current";
            }
          }
          if(twai_receive(&message_inverter_speed_rpm_temp, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_inverter_speed_rpm_temp.identifier == 0x18FE8D02){
                printf("\nID Inverter: %x", message_inverter_speed_rpm_temp.identifier);        
                uint8_t input_voltage_msb = message_inverter_speed_rpm_temp.data[2];
                uint8_t input_voltage_lsb = message_inverter_speed_rpm_temp.data[1];
                pdu_input_voltage = (input_voltage_msb << 8) | input_voltage_lsb;
                // int real_speed = engine_speed * 0.125;
                input_pdu_tegangan = waktu + " " + pdu_input_voltage + "\t\t" + "pdu_input_voltage";

                uint8_t output_voltage_msb = message_inverter_speed_rpm_temp.data[4];
                uint8_t output_voltage_lsb = message_inverter_speed_rpm_temp.data[3];
                pdu_output_voltage = (output_voltage_msb << 8) | output_voltage_lsb;
                // int real_speed = engine_speed * 0.125;
                output_pdu_tegangan = waktu + " " + pdu_output_voltage + "\t\t" + "pdu_output_voltage";

                uint8_t pdu_dcdc_current_msb = message_inverter_speed_rpm_temp.data[6];
                uint8_t pdu_dcdc_current_lsb = message_inverter_speed_rpm_temp.data[5];
                pdu_dcdc_current = (pdu_dcdc_current_msb << 8) | pdu_dcdc_current_lsb;
                // int real_rpm = engine_rpm * 0.125;
                pdu_dcdc_arus = waktu + " " + pdu_dcdc_current + "\t\t" + "pdu_dcdc_current";
                // if(currentMillis - prevMillis >= 7000){
                //   File file = SD.open(inverter_pdu_state_now_cstr, FILE_APPEND);
                //   if (!file) {
                //     Serial.println("File not exist yet");
          ...
          if(twai_receive(&message_inverter_speed_rpm_temp, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_inverter_speed_rpm_temp.identifier == 0x18FE8D01){
                printf("\nID Inverter: %x", message_inverter_speed_rpm_temp.identifier);        
                ...
                pdu_output_voltage = (output_voltage_msb << 8) | output_voltage_lsb;
                // int real_speed = engine_speed * 0.125;
                pdu_hv = waktu + " " + pdu_output_voltage + "\t\t" + "pdu_output_voltage";

                uint8_t pdu_dcdc_current_msb = message_inverter_speed_rpm_temp.data[6];
                uint8_t pdu_dcdc_current_lsb = message_inverter_speed_rpm_temp.data[5];
                pdu_dcdc_current = (pdu_dcdc_current_msb << 8) | pdu_dcdc_current_lsb;
                // int real_rpm = engine_rpm * 0.125;
                pdu_current = waktu + " " + pdu_dcdc_current + "\t\t" + "pdu_dcdc_current";
                // if(currentMillis - prevMillis >= 7000){
                //   File file = SD.open(inverter_pdu_state_now_cstr, FILE_APPEND);
                //   if (!file) {
                //     Serial.println("File not exist yet");
          ...
          if(twai_receive(&message_inverter_speed_rpm_temp, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_inverter_speed_rpm_temp.identifier == 0x18FE8D03){
                printf("\nID Inverter: %x", message_inverter_speed_rpm_temp.identifier);        
                uint8_t pdu_voltage_msb = message_inverter_speed_rpm_temp.data[2];
                uint8_t pdu_voltage_lsb = message_inverter_speed_rpm_temp.data[1];
                pdu_voltage = (pdu_voltage_msb << 8) | pdu_voltage_lsb;
                // int real_speed = engine_speed * 0.125;
                pdu_tegangan = waktu + " " + pdu_voltage + "\t\t" + "pdu_voltage";
                // if(currentMillis - prevMillis >= 7000){
                //   File file = SD.open(inverter_pdu_state_now_cstr, FILE_APPEND);
                //   if (!file) {
                //     Serial.println("File not exist yet");
      ...
      if(write_or_not == 1){
        if(twai_receive(&message_dcdc, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_dcdc.identifier == 0x18F11401){
                ...
                current = waktu + " " + String(dcdc_current) + "\t\t" + "dcdc_current";
                ...
                hv_input = waktu + " " + String(dcdc_hv_input) + "\t\t" + "dcdc_hv_input";
                ...
                lv_output = waktu + " " + String(dcdc_lv_output) + "\t\t" + "dcdc_lv_output";
                // if(currentMillis - prevMillis >= 7000){
          ...
          if(twai_receive(&message_bms_cell1to4, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_bms_cell1to4.identifier == 0x10F51403){
                ...
                cvoltage1 = waktu + " " + "1" + "\t" + String(cell_voltage1) + "\t\t" + "cell_voltage1";
                ...
                cvoltage2 = waktu + " " + "2" + "\t" + String(cell_voltage2) + "\t\t" + "cell_voltage2";
                ...
                cvoltage3 = waktu + " " + "3" + "\t" + String(cell_voltage3) + "\t\t" + "cell_voltage3";
                ...
                cvoltage4 = waktu + " " + "4" + "\t" + String(cell_voltage4) + "\t\t" + "cell_voltage4";
                // if(currentMillis - prevMillis >= 7000){
                // File file = SD.open(dcdc_bms_state_now_cstr, FILE_APPEND);
                //   if (!file) {
                //     Serial.println("File not exist yet");
      ...
      if(write_or_not == 1){
        if(twai_receive(&message_batt_charger, pdMS_TO_TICKS(1000)) == ESP_OK){
              if(message_batt_charger.identifier == 0x00FD15){
                ...
                batt_voltage = waktu + " " + String(batt_charge_voltage) + "\t\t" + "batt_charge_voltage";
                ...
                batt_current = waktu + " " + String(batt_charge_current) + "\t\t" + "batt_charge_current";
...
void setup(){
  ...
  hspi->endTransaction();
  delay(5000); //used to ensure if slave has accessed the sd card
  if(!SD.begin(5)){
