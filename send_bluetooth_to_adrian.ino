...
void readFileBulk(fs::FS &fs, long int start_date, long int end_date, int pilihan) {
  // Serial.printf("Reading file: %s\n", path);
  // slave.end();
  const char* tempat_file;
  digitalWrite(32, HIGH);
  ...
  switch(pilihan){
    ...
    case 12:
      ...
      break;
    case 13:
      nama_file = "_battTempChargeState.txt";
      break;
  }
  ...
  for(long int epoch = start_date;epoch <= end_date;epoch+=86400){
    ...
    Serial.print("Read from file in bulk: ");
    if(file){
      while(file.available()) {
        // file.read();
        hasil_baca_file = file.readStringUntil('\n');
        ...
        // SerialBT.println();
        // hasil_baca_file = file.readString();
        int line = hasil_baca_file.indexOf("\t\t");
        if(line != -1){
          // hasil_baca_file.replace(filter, "");
          // hasil_baca_file.replace("\t\t", "");
          hasil_baca_file = hasil_baca_file.substring(0, line);
          hasil_baca_file = hasil_baca_file + "\n";          
          SerialBT.print(hasil_baca_file);
          hasil_baca_file = "";
          SerialBT.flush();
...
void read_self_check(fs::FS &fs){
  ...
  digitalWrite(27, HIGH);
  digitalWrite(4, HIGH);
  File file = fs.open("/self_check.txt", FILE_READ);
  if(file){
    while(file.available()){
...
void clear_dtc(){
  ...
  File file = SD.open("/dtc_all_now.txt", FILE_WRITE);
  if(file){
    file.println("");
  }
  file.close();
      ...
      if (incomingChar != '\n') {
        ...
        if (!message.isEmpty()) {
          ...
          if(message.equals("SELF,CHECK,")){
            check_enable = 1;
            SerialBT.println("SELF CHECK READY");
            // read_self_check();
          }
          if(message.equals("SELF,CHECK,RETRIEVAL,READY,")){
            if(check_enable == 1){
              check_enable = 2;
            }
          }
          if(message.equals("SELF,CHECK,RETRIEVAL,ACK,")){
            if(check_enable == 0){
              SerialBT.print("DONE\r\n");
              SerialBT.print("\n");
              SerialBT.print("\n");
...
void process_request(long int waktu_mulai, long int waktu_akhir, String request){
  ...
  switch(kiriman){
    ...
    //   break;
    case 11:
      SerialBT.print("Bulk data inverter PDU");
      readFileBulk(SD, waktu_mulai, waktu_akhir, 11);
      break;
    case 12:
      SerialBT.print("Bulk data dcdc bms");
      readFileBulk(SD, waktu_mulai, waktu_akhir, 12);
      break;
    case 13:
      SerialBT.print("Bulk data batre");
      readFileBulk(SD, waktu_mulai, waktu_akhir, 13);
      break;      
  }
...
void onBTConnect(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) {
  ...
  else if (event == ESP_SPP_CLOSE_EVT || event == ESP_SPP_SRV_STOP_EVT) {
      Serial.println("Device or Server disconnected!");
      allowed = 0;
...
void loop() {
  ...
  if (check_enable == 2 && enableMillis == 0) {
    long int currentTime = millis();
